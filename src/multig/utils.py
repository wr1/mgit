...
def fold_files(files: List[Path], output: Path) -> None:
    ...
    try:
        ...
        fold(files=[str(f) for f in files], output=str(output), prompt=None, dialect="default", bare=False)
    except ImportError:
        # Fallback to cat
...
def resolve_targets(config: dict, targets: List[str], root: Path) -> tuple[Set[str], Set[Path]]:
