...
def status(root: Path, profile: Optional[str] = None) -> None:
    ...
    for repo in repo_paths:
        ...
        branch = run_command(["git", "branch", "--show-current"], cwd=repo).stdout.strip()
        dirty = "Yes" if run_command(["git", "status", "--porcelain"], cwd=repo).stdout else "No"
        ahead_behind = run_command(["git", "status", "-b", "--porcelain"], cwd=repo).stdout.split()[1:3]
...
def branch(root: Path, name: str, delete: bool = False, sync: bool = False, profile: Optional[str] = None) -> None:
    """Manage branches."""
    config = load_config(root)
    repos = config["profiles"].get(profile or "all", [])
    repo_paths = get_repo_paths(root, repos)
    if delete:
        ...
        current = run_command(["git", "branch", "--show-current"], cwd=root / repos[0]).stdout.strip()
...
def test_repos(root: Path, glob: Optional[str] = None, parallel: bool = True, profile: Optional[str] = None) -> None:
...
def commit_repos(root: Path, all: bool = False, message: str = "Auto commit new files", profile: Optional[str] = None) -> None:
    ...
    for repo in repo_paths:
        ...
        if all:
            ...
            result_add = run_command(["git", "add", "*.py", "pyproject.toml", "README.md"], cwd=repo)
            ...
            for path in paths:
                if (repo / path).exists():
                    ...
                    if result_add.returncode == 0:
                        ...
                        logger.warning(f"Failed to add {path} in {repo.name}: {result_add.stderr}")
            if added_any:
        ...
        if result_staged.returncode == 0:
            ...
            continue
        logger.info(f"Committing in {repo.name}")
        result_commit = run_command(["git", "commit", "-m", message, "--no-edit"], cwd=repo)
        if result_commit.returncode != 0:
            logger.error(f"Failed to commit in {repo.name}: {result_commit.stderr or result_commit.stdout}")
        else:
            logger.info(f"Committed in {repo.name}")
            # Show diff
            diff_result = run_command(["git", "show", "--stat"], cwd=repo)
...
def push_repos(root: Path, profile: Optional[str] = None) -> None:
    ...
    for repo in repo_paths:
        ...
        upstream_check = run_command(["git", "rev-parse", "--abbrev-ref", f"{branch}@{{upstream}}"], cwd=repo)
        if upstream_check.returncode != 0:
            # No upstream, set it
            logger.info(f"Setting upstream for {branch} in {repo.name}")
            result = run_command(["git", "push", "--set-upstream", "origin", branch], cwd=repo)
        else:
